...
export function AuthForm({ mode }: AuthFormProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      toast.error("Supabase environment variables are missing. Please check your .env.local file.");
      setLoading(false);
      return;
    }
      ...
      <CardContent className="space-y-4 pt-4">
        <div className="grid grid-cols-1 gap-4">
          <Button 
            ...
            className="w-full py-6 rounded-2xl border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex gap-3 font-bold"
          >
            {googleLoading ? (
